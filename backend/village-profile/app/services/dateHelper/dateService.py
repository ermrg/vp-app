...
from app.services.dateHelper.constants import month_name_dev

__all__ = ['samwat', 'convert_ad_to_bs', 'convert_bs_to_ad', 'nepali_date_from_string', 'get_start_end_date_of_bs_in_ad']
...
def nepali_date_from_string(date_string, date_format):
    dates = list(filter(None, split([', ', ' ', '-'], date_string)))
    date_formats = list(filter(None, split([',', ' ', '-'], date_format)))
    y = ''
    m = ''
    day = ''
    for index, d in enumerate(date_formats):
        if d == 'd':
            day = dates[index]

        if d == 'M':
            m = month_name_dev.index(dates[index])
    ...
    if day and m and y:
        return samwat(int(y), int(m), int(day))
...
def get_start_end_date_of_bs_in_ad(filter_):  # get_start_end_date_of_current_nepali_month_in_ad
    today_bs = samwat.today()
    today_ad = date.today()
    if filter_ == 'monthly':
        start_date_ad = convert_bs_to_ad(samwat(today_bs.year, today_bs.month, 1))
        if today_bs.month != 12:
            end_date_ad = convert_bs_to_ad(samwat(today_bs.year, today_bs.month + 1, 1)) - timedelta(days=1)
        else:
            end_date_ad = convert_bs_to_ad(samwat(today_bs.year + 1, 1, 1)) - timedelta(days=1)

    elif filter_ == 'yearly':
        start_date_ad = convert_bs_to_ad(samwat(today_bs.year, 1, 1))
        end_date_ad = convert_bs_to_ad(samwat(today_bs.year + 1, 1, 1)) - timedelta(days=1)
    elif filter_ == 'weekly':
        weekday = 0 if today_ad.weekday() == 6 else today_ad.weekday() + 1
        start_date_ad = today_ad - timedelta(days=weekday)
        end_date_ad = start_date_ad + timedelta(days=6)
    elif filter_ == 'daily':
        start_date_ad = today_ad
        end_date_ad = ''
    else:
        start_date_ad = ''
        end_date_ad = ''

    data = {'start_date_ad': start_date_ad, 'end_date_ad': end_date_ad}
    return data
