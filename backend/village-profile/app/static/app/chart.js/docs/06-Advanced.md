...
```javascript
// Destroys a specific analytics instance
myLineChart.destroy();
```
...
// => returns HTML string of a legend for this analytics
```
...
Chart.Type.extend({
	...
	initialize:  function(data){
		this.chart.ctx // The drawing context for this analytics
		this.chart.canvas // the canvas node for this analytics
	},
...
// Now we can create a new instance of our analytics, using the Chart.js API
...
```javascript
// Notice now we're extending the particular Line analytics type, rather than the base class.
Chart.types.Line.extend({
	// Passing in a name registers this analytics in the Chart namespace in the same way
	name: "LineAlt",
	initialize: function(data){
		console.log('My Line analytics extension');
		Chart.types.Line.prototype.initialize.apply(this, arguments);
	}
});

// Creates a line analytics in the same way
new Chart(ctx).LineAlt(data);
// but this logs 'My Line analytics extension' in the console.
```
